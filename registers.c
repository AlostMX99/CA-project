...
void initRegisterHome() {
    ...
    for (int i = 1; i <= 31; i++) {
        ...
        myRegisters->regHome[i - 1].type = strdup(name);
        myRegisters->regHome[i - 1].data = 0;
    }
    myRegisters->regHome[31].type = strdup("zero");
    myRegisters->regHome[31].data = 0;

    myRegisters->regHome[32].type = strdup("pc");
    myRegisters->regHome[32].data = 0;
}
